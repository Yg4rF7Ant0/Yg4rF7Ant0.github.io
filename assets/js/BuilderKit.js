...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (courseLoadingMessage) courseLoadingMessage.style.display = "none";
  }
});
...
function renderTimelineItems(items, containerId) {
  ...
  items.forEach((item) => {
    ...
    // fallback value untuk mencegah undefined
    ...
    // credential rendering aman
    const credentialHtml = credentialUrl
      ? `
        <div class="timeline-credential">
          <span class="timeline-item-date">${date}</span>
          <a href="${credentialUrl}" target="_blank" rel="noopener noreferrer" class="timeline-credential-url">
            Show credential
          </a>
        </div>
        `
    ...
    li.innerHTML = `
      ...
      <span class="timeline-item-organization">${organization}</span>
      ${credentialHtml}
      <p class="timeline-text">${skill}</p>
    `;
